...
for contest in $contests; do
    ...
    if [ -f $base_url_0/PLAYER1 ] && [ -f $base_url_0/PLAYER2 ] && [ -f $base_url_0/PLAYER3 ] && [ -f $base_url_0/PLAYER4 ] && [ -f $base_url_1/PLAYER1 ] && [ -f $base_url_1/PLAYER2 ] && [ -f $base_url_1/PLAYER3 ] && [ -f $base_url_1/PLAYER4 ]; then
        if [ "$cnt" -lt "$2" ]; then
            mkdir /data/thuai4_final/${teams[0]}_vs_${teams[1]}

            docker network create ${teams[0]}_vs_${teams[1]}

            docker run --name=THUAI4_final_server_${teams[0]}_vs_${teams[1]} --network=${teams[0]}_vs_${teams[1]} -d --rm -v /data/thuai4_final/${teams[0]}_vs_${teams[1]}:/usr/local/mnt eesast/thuai_server:latest 1200 2 4 0 0 1
