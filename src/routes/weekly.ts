...
router.post("/check", authenticate(["counselor"]), async (req, res) => {
  try {
    ...
    const base_directory = await utils.get_base_directory();
    if (
      await utils.checkPathExists(
        `${base_directory}/weixinSpiderStatus/${filename}`,
      )
    ) {
...
router.get("/cover", async (req, res) => {
  try {
    if (!req.query.url)
      return res.status(400).send("400 Bad Request: no url provided!");
    ...
    if (response.ok) {
      ...
      if (match && match[1]) res.status(200).send(match[1]);
      else throw Error("capture failed!");
    } else
      return res.status(500).send("500 Internal Server Error: fetch failed!");
  } catch (err) {
    return res.status(500).send("500 Internal Server Error: " + err);
  }
});
...
router.post("/insert", authenticate(["counselor"]), async (req, res) => {
  try {
    if (!req.body.id || !req.body.url)
      return res.status(400).send("400 Bad Request: not enough params!");
...
router.post("/init", authenticate(["counselor"]), async (req, res) => {
  try {
    if (!req.body.data)
      return res.status(400).send("400 Bad Request: no data provided!");
    await client.request(
      gql`
