...
router.post("/add_course", authenticate(["admin"]), async (req, res) => {
  try {
    const { year, type, semester, professor, name, language, fullname, code } =
      req.body;
    if (
      !year ||
      !type ||
    ...
    const course_uuid = await ContHasFunc.add_course(
      year,
      type,
      semester,
      professor,
    ...
    res
      .status(200)
      .json({ course_uuid: course_uuid, message: "Course added successfully" });
  } catch (err: any) {
    res.status(500).json({
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const course_info_id = await ContHasFunc.add_course_info(
        key,
        value,
        course_id,
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      const { dim1, dim2, dim3, dim4, dim5, dim6, course_id, user_uuid } =
        req.body;
      if (
        !dim1 ||
        !dim2 ||
      ...
      const created_at = await ContHasFunc.add_course_rating(
        dim1,
        dim2,
        dim3,
        dim4,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const comment_uuid = await ContHasFunc.add_course_comment_one(
        comment,
        user_uuid,
        course_uuid,
        parent_uuid,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await ContHasFunc.add_course_comment_stars(
        comment_uuid,
        user_uuid,
      );
      if (!result) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await ContHasFunc.add_course_comment_likes(
        comment_uuid,
        user_uuid,
      );
      if (!result) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await ContHasFunc.update_course(
        code,
        uuid,
        fullname,
        language,
      ...
      res
        .status(200)
        .json({ uuid: result, message: "Course updated successfully" });
    } catch (err: any) {
      res.status(500).json({
        error: "500 Internal Server Error",
        message: err.message,
...
router.post(
  "/update_course_info",
  authenticate(["counselor", "professor"]),
  async (req, res) => {
    try {
      const { course_id, old_key, new_key, value } = req.body;
      if (!course_id || !old_key || !new_key) {
        return res
          .status(400)
          .json({ error: "400 Bad Request: Missing required parameters" });
      }
      const affected_rows = await ContHasFunc.update_course_info(
        course_id,
        old_key,
        new_key,
        value,
      ...
      if (affected_rows === 0) {
        return res
          .status(404)
          .json({ message: "Course info not found or no changes made" });
      }
      res.status(200).json({ message: "Course info updated successfully" });
    } catch (err: any) {
      // 捕获主键冲突错误（例如 new_key 已经存在）
      if (err.message && err.message.includes("Uniqueness violation")) {
...
router.post(
  ...
  async (req, res) => {
    try {
      const { course_id, user_uuid, dim1, dim2, dim3, dim4, dim5, dim6 } =
        req.body;
      if (
        !course_id ||
        !user_uuid ||
      ...
      const result = await ContHasFunc.update_course_rating(
        course_id,
        user_uuid,
        dim1,
        dim2,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ uuid: result, message: "Comment updated successfully" });
    } catch (err: any) {
      res.status(500).json({
        error: "500 Internal Server Error",
        message: err.message,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ uuid: result, message: "Course deleted successfully" });
    } catch (err: any) {
      res.status(500).json({
        error: "500 Internal Server Error",
        message: err.message,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await ContHasFunc.delete_course_rating(
        course_id,
        user_uuid,
      );
      if (!result) {
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ uuid: result, message: "Comment deleted successfully" });
    } catch (err: any) {
      res.status(500).json({
        error: "500 Internal Server Error",
        message: err.message,
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      const result = await ContHasFunc.delete_course_comment_stars(
        comment_uuid,
        user_uuid,
      );
      if (!result) {
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      const result = await ContHasFunc.delete_course_comment_likes(
        comment_uuid,
        user_uuid,
      );
      if (!result) {
