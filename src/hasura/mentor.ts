...
export const get_mentor_applications_count = async (
  uuid: string,
  year: number,
) => {
  const query: any = await client.request(
...
export const get_mentor_applications_approved_count = async (
  uuid: string,
  year: number,
) => {
  const query: any = await client.request(
...
export const insert_mentor_application = async (
  mentor_uuid: string,
  student_uuid: string,
  year: number,
  statement: string,
...
export const update_mentor_info_available = async (
  uuid: string,
  available: boolean,
) => {
  const query: any = await client.request(
...
export const update_mentor_info_description = async (
  mentor_uuid: string,
  ...
  const setFields: any = {};
  if (updateFields.achievement)
    setFields.achievement = updateFields.achievement;
...
export const update_mentor_application_status = async (
  id: string,
  status: string,
) => {
  const query: any = await client.request(
...
export const update_mentor_application_statement = async (
  id: string,
  statement: string,
) => {
  const query: any = await client.request(
    gql`
      mutation UpdateMentorApplicationStatement(
        $id: uuid!
        $statement: String!
      ) {
        update_mentor_application_by_pk(
...
export const update_mentor_application_chat_status = async (
  id: string,
  chat_status: boolean,
) => {
  try {
...
export const insert_mentor_application_schedule = async (
  year: number,
  start_A: Date,
  start_B: Date,
  start_C: Date,
...
export const insert_freshman_info_list = async (
  fresman_data: Freshman_Insert_Input,
) => {
  const query: any = await client.request(
    gql`
      mutation InsertFreshmanInfoList(
        $freshmanData: [freshman_insert_input!]!
      ) {
        insert_freshman(
          objects: $freshmanData
          on_conflict: {
...
export const get_current_semester = async () => {
  const query: any = await client.request(gql`
    query fetch_current_semester {
      semester(where: { is_current: { _eq: true } }) {
        semester
      }
    }
  `);

  return query.semester?.[0]?.semester ?? null;
};

export const get_semester_list = async () => {
  const query: any = await client.request(gql`
    query fetch_semester_list {
      semester(order_by: { semester: desc }) {
        semester
        is_current
      }
    }
...
export const add_semester = async (semester: string) => {
  const query: any = await client.request(
    gql`
      mutation add_semester($semester: String!) {
        insert_semester_one(
          object: { semester: $semester, is_current: false }
          on_conflict: { constraint: semester_pkey, update_columns: [] }
        ) {
          semester
        }
      }
  ...
  return query.insert_semester_one?.semester ?? null;
};

export const delete_semester = async (semester: string) => {
  const query: any = await client.request(
    gql`
      mutation delete_semester($semester: String!) {
        delete_semester_by_pk(semester: $semester) {
          semester
        }
      }
    `,
  ...
  return query.delete_semester_by_pk?.semester ?? null;
};

export const set_current_semester = async (semester: string) => {
  const query: any = await client.request(
    gql`
      mutation set_current_semester($semester: String!) {
        reset_current: update_semester(where: {}, _set: { is_current: false }) {
          affected_rows
        }
        set_current: update_semester(
          where: { semester: { _eq: $semester } }
  ...
  return query.set_current?.affected_rows ?? null;
};
