...
export const add_course = async (
  ...
  const add_course_query: {
    insert_course?: { returning: { uuid: string }[] };
 ...
 */
export const add_course_info = async (
  key: string,
  value: string,
  course_id: string,
): Promise<string | undefined> => {
  const add_course_info_query: {
    insert_course_info_one?: { course_id: string };
  } = await client.request(
    gql`
      mutation AddCourseInfo(
        $key: String!
        $value: String!
        $course_id: uuid!
      ) {
 ...
 */
export const add_course_rating = async (
  dim1: number,
  dim2: number,
  dim3: number,
  dim4: number,
  ...
  const add_course_rating_query: {
    insert_course_rating_one?: { created_at: string };
  } = await client.request(
    gql`
 ...
 */
export const add_course_comment_one = async (
  comment: string,
  user_uuid: string,
  course_uuid: string,
  parent_uuid: string,
): Promise<string | undefined> => {
  const add_course_comment_one_query: {
    insert_course_comment_one?: { uuid: string };
  } = await client.request(
    gql`
 ...
 */
export const add_course_comment_stars = async (
  comment_uuid: string,
  user_uuid: string,
): Promise<string | undefined> => {
  const add_course_comment_stars_query: {
    insert_course_comment_stars_one?: { comment_uuid: string };
  } = await client.request(
    gql`
  ...
  return add_course_comment_stars_query?.insert_course_comment_stars_one
    ?.comment_uuid;
};
 ...
 * @returns {Promise<string | undefined>} - The UUID of the comment
 */
export const add_course_comment_likes = async (
  comment_uuid: string,
  user_uuid: string,
): Promise<string | undefined> => {
  const add_course_comment_likes_query: {
    insert_course_comment_likes_one?: { comment_uuid: string };
  } = await client.request(
    gql`
  ...
  return add_course_comment_likes_query?.insert_course_comment_likes_one
    ?.comment_uuid;
};
...
export const update_course = async (
  ...
  const update_course_query: { update_course_by_pk?: { uuid: string } } =
    await client.request(
      gql`
...
/**
 * Update course info key and value
 * @param {string} course_id - The UUID of the course
 * @param {string} old_key - The current key
 * @param {string} new_key - The new key to set
 * @param {string} value - The new value to set
 * @returns {Promise<number | undefined>} - The number of affected rows
 */
export const update_course_info = async (
  course_id: string,
  old_key: string,
  new_key: string,
  value: string,
): Promise<number | undefined> => {
  const update_query: { update_course_info?: { affected_rows: number } } =
    await client.request(
      gql`
        mutation UpdateCourseInfoAtomic(
          $course_id: uuid!
          $old_key: String!
          $new_key: String!
          $value: String!
          ...
          update_course_info(
            where: { course_id: { _eq: $course_id }, key: { _eq: $old_key } }
            _set: { key: $new_key, value: $value }
          ) {
            affected_rows
          }
        }
  ...
  return update_query?.update_course_info?.affected_rows;
};
 ...
 */
export const update_course_rating = async (
  course_id: string,
  user_uuid: string,
  dim1: number,
  dim2: number,
  ...
  const update_course_rating_query: {
    update_course_rating_by_pk?: { updated_at: string };
  } = await client.request(
    gql`
 ...
 */
export const delete_course = async (
  uuid: string,
): Promise<string | undefined> => {
  const delete_course_query: { delete_course_by_pk?: { uuid: string } } =
    await client.request(
      gql`
 ...
 */
export const delete_course_info = async (
  course_id: string,
  key: string,
): Promise<{ course_id: string; key: string } | undefined> => {
  const delete_course_info_query: {
    delete_course_info_by_pk?: { course_id: string; key: string };
  } = await client.request(
    gql`
 ...
 */
export const delete_course_rating = async (
  course_id: string,
  user_uuid: string,
): Promise<{ course_id: string; user_uuid: string } | undefined> => {
  const delete_course_rating_query: {
    delete_course_rating_by_pk?: { course_id: string; user_uuid: string };
  } = await client.request(
    gql`
      mutation DeleteCourseRating($course_id: uuid!, $user_uuid: uuid!) {
        delete_course_rating_by_pk(
          course_id: $course_id
          user_uuid: $user_uuid
        ) {
          course_id
 ...
 */
export const delete_course_comment_one = async (
  uuid: string,
): Promise<string | undefined> => {
  const delete_course_comment_one_query: {
    update_course_comment_by_pk?: { uuid: string };
  } = await client.request(
    gql`
 ...
 * @returns {Promise<string | undefined>} - The UUID of the comment
 */
export const delete_course_comment_stars = async (
  comment_uuid: string,
  user_uuid: string,
): Promise<string | undefined> => {
  const delete_course_comment_stars_query: {
    delete_course_comment_stars_by_pk?: { comment_uuid: string };
  } = await client.request(
    gql`
      mutation DeleteCourseCommentStars(
        $comment_uuid: uuid!
        $user_uuid: uuid!
      ) {
  ...
  return delete_course_comment_stars_query?.delete_course_comment_stars_by_pk
    ?.comment_uuid;
};
 ...
 * @returns {Promise<string | undefined>} - The UUID of the comment
 */
export const delete_course_comment_likes = async (
  comment_uuid: string,
  user_uuid: string,
): Promise<string | undefined> => {
  const delete_course_comment_likes_query: {
    delete_course_comment_likes_by_pk?: { comment_uuid: string };
  } = await client.request(
    gql`
      mutation DeleteCourseCommentLikes(
        $comment_uuid: uuid!
        $user_uuid: uuid!
      ) {
  ...
  return delete_course_comment_likes_query?.delete_course_comment_likes_by_pk
    ?.comment_uuid;
};

export const get_newest_weekly = async (): Promise<Date> => {
  const get_newest_weekly_query: any = await client.request(gql`
    query MyQuery {
      weekly_aggregate {
        aggregate {
          max {
  ...
  `);
  const date =
    get_newest_weekly_query?.weekly_aggregate?.aggregate?.max?.date +
    "T00:00:00.000+08:00";
...
export const add_weekly_list = async (
  weekly_list: WeeklyPost[],
): Promise<string | undefined> => {
...
export const get_newest_weekly_id = async (): Promise<number> => {
  const get_newest_weekly_id_query: any = await client.request(gql`
    query MyQuery {
      weekly_aggregate {
        aggregate {
          max {
